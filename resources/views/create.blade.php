...
<form action="{{ route('admin.person.store') }}" method="post" class="container mt-4 mb-4">
    @csrf
    ...
    <div class="form-group">
        ...
        <select name="parent_id" id="parent_id" class="form-control">
            ...
            @foreach($people as $person)
                <option value="{{ $person->id }}" @if(old('parent_id') == $person->id) selected @endif>{{ $person->name }}</option>
            @endforeach
        </select>
        @error('parent_id')
        <div class="error-validate mt-3">{{ $message }}</div>
    ...
    <div class="form-group mt-4">
        <button type="submit" class="btn btn-primary">Thêm người </button>
    </div>
</form>
@if (session('success'))
<script>
